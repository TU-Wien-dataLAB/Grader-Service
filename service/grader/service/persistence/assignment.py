...
def create_assignment(assignment: Assignment, lectid: int):
    engine = create_engine(DataBaseManager.get_database_url(), echo=True)
    session = Session(bind=engine)
    insert = 'INSERT INTO "assignment" ("name","lectid","duedate","path","points","status") VALUES ("%s",%i,"%s","%s",%i,"%s")' % (assignment.name, lectid, assignment.due_date, assignment.path, assignment.points, assignment.status) 
...
def update_assignment(assignment: Assignment):
    engine = create_engine(DataBaseManager.get_database_url(), echo=True)
    session = Session(bind=engine)
    update = 'UPDATE "assignment" SET name=%s, duedate=%s, path=%s, points=%i, status=%i WHERE id=%i' % (assignment.name, assignment.due_date, assignment.path, assignment.points, assignment.status,assignment.id)
...
def delete_assignment(assignid: Assignment):
    engine = create_engine(DataBaseManager.get_database_url(), echo=True)
    session = Session(bind=engine)
    delete = 'DELETE FROM "assignment" WHERE id=%i' % assignid
