...
class Submission(Base, Serializable):
  ...
  def model(self) -> submission.Submission:
      return submission.Submission(id=self.id, submitted_at=(self.date.isoformat("T", "milliseconds") + "Z"), status=self.status, score=self.score,commit_hash=self.commit_hash)
