...
class JupyterHubGroupAuthenticator(TokenAuthenticator):
    hub_api_url = Unicode(os.environ.get("JUPYTERHUB_API_URL"),
                          allow_none=False).tag(config=True)

    group_separator = Unicode(":",
                              help="Separator for splitting lecture codes \
        and user roles in JupyterHub group name.",
    ...
    def __init__(self, **kwargs):
        ...
        self.hub_request_service = RequestService(
            url=f"{hub_api_parsed.scheme}://{hub_api_parsed.netloc}")
    ...
    async def get_user(self, handler: GraderBaseHandler) \
        ...
        if user is None:
            ...
            raise HTTPError(403)
        self.set_cookie(handler, token, user,
                        minutes_valid=self.max_token_cookie_age_minutes)
        ...
        lecture_roles = {
            ...
            for code, role in
            (t for t in
             (tuple(g.split(self.group_separator, 1)) for g in user["groups"])
             if len(t) == 2)
        }

        return user_model, lecture_roles

    async def authenticate_token_user(self, handler: GraderBaseHandler,
                                      token: str) -> Optional[dict]:
        token_user = self.get_cookie(
            handler, token, minutes_valid=self.max_token_cookie_age_minutes)
        if not token_user:
    ...
    def authenticate_cookie_user(self, handler: GraderBaseHandler,
                                 user: dict) -> bool:
        cookie_user = self.get_cookie(
            handler, user["name"],
            minutes_valid=self.max_user_cookie_age_minutes)
        if not cookie_user:
            self.set_cookie(
    ...
    async def get_current_user_async(self, token) -> Optional[dict]:
        try:
            ...
            self.log.error(
                f"Could not connect to the JupyterHub at {self.hub_api_url}! "
