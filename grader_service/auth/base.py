...
class Authenticator(LoggingConfigurable):
    use_cookies = Bool(
        True,
        help="Whether to store cookies when authenticating users.",
        config=True, allow_none=False)
    max_user_cookie_age_minutes = Integer(
        15,
        help="Time in minutes until a user cookie expires.",
        config=True,
        allow_none=False)
    max_token_cookie_age_minutes = Integer(
        5,
        help="Time in minutes until a token cookie expires.",
        config=True,
        allow_none=False)
    ...
    async def authenticate_user(self, handler: GraderBaseHandler) -> None:
        """ Authenticates a user by setting the ``current_user`` property
        of the request handler based on the request.
        Adds the necessary user roles for the user.

        :param handler: Tornado request handler with active sqlalchemy session.
        :return: None
        """
        start_time = time.monotonic()
        user, lecture_roles = await self.get_user(handler)
        self.log.info(
            f'User {user.name} has been authenticated \
            (took {(time.monotonic() - start_time) * 1e3:.2f}ms)')

        if self.get_cookie(
                handler, user.name,
                minutes_valid=self.max_user_cookie_age_minutes) is None:
            self.set_lecture_roles(handler, user, lecture_roles)
        handler.current_user = user

    async def get_user(self, handler: GraderBaseHandler) \
            -> Tuple[User, Dict[str, Dict[str, str]]]:
        """Retrieves a user based on the request from an authentication
        provider and builds the lecture roles based on the authentication
        result.
        ...
        :param handler: Tornado request handler with active sqlalchemy session.
        :return: A tuple of the user and lecture roles the user should have.
        """
        raise NotImplementedError("Implement in subclass!")

    def set_lecture_roles(self, handler: GraderBaseHandler, user: User,
                          lecture_roles: Dict[str, Dict[str, str]]):
        ...
        for lecture_code, obj in lecture_roles.items():
            ...
            if lecture is None:
                msg = f"Could not find lecture with code: {lecture_code}. \
    ...
    def get_or_create_user_model(self, handler: GraderBaseHandler,
                                 user_name: str) -> User:
        user_model = handler.session.query(User).get(user_name)
        if user_model is None:
            self.log.info(
                f'User {user_name} does not exist and will be created.')
            user_model = User()
    ...
    def set_cookie(self, handler: GraderBaseHandler, name: str,
                   data: Union[str, dict], minutes_valid: float) -> None:
        """
    ...
    def get_cookie(self, handler: GraderBaseHandler, name: str,
                   minutes_valid: float, decode=False) \
            -> Union[str, dict, None]:
        """ Get a cookie stored in the handler. Takes care of quoting
        ...
        if not self.use_cookies:
            return None
        expires_days = minutes_valid / 1440
        data = handler.get_secure_cookie(quote(name),
                                         max_age_days=expires_days)
        if data is None:
            return None
        if decode:
...
class TokenAuthenticator(Authenticator):
    """Base class for authenticator that use tokens in request
    for authentication. Also supports basic auth but ignores username and
    interprets the password as a token!
    """
        ...
        for (k, v) in sorted(handler.request.headers.get_all()):
            if k == "Authorization":
                ...
                if name == "Token":
                    ...
                    # we interpret the password as the token
