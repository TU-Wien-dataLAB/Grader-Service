...
import { Button as MuiButton, Alert, AlertTitle, Box, Card, Dialog, DialogActions, DialogContent, DialogTitle, DialogContentText,Typography, Paper, Chip, Grid, CardHeader, CardContent, ListItem, List, Stack } from "@mui/material";
...
export const Validator = (props: ValidatorProps) => {
    ...
    const validateNotebook = () => {
        ...
        props.notebook.widgets.map((c: Cell) => {
            const metadata: NbgraderData = CellModel.getNbgraderData(c.model.metadata);
            ...
            if (metadata !== null) {
                if (metadata.grade_id !== null) {
                    if (ids.has(metadata.grade_id)) {
                        ...
                        result.push({ id: metadata.grade_id, type: "error", msg: "Duplicate ID found" });
                ...
                result.push({ id: "Warning", type: "warning", msg: "Cell with no type found"});
            }
